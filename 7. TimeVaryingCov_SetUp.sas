...
TASK:
data set up of time-varying covariate analysis
...
proc freq data=totpop_os4 noprint; 
...
*drop those where stop_date1 doesn't match infection date;
data infection_only2; set infection_only; 
where Stop_date1 = infect_start; 
run;
...
data tot_pop_long3; set tot_pop_long2;
	...
	if viral_infec = 1 and any_infec2 = 1 then viral_infec2 = 1; 
	else if viral_infec = 1 and any_infec2 = 0 then viral_infec2 = 0; 
	else if viral_infec = 0 then viral_infec2 = 0; 

	if bacterial_infec = 1 and any_infec2 = 1 then bacterial_infec2 = 1; 
	else if bacterial_infec = 1 and any_infec2 = 0 then bacterial_infec2 = 0; 
	else if bacterial_infec = 0 then bacterial_infec2 = 0; 

	if parasitic_infec = 1 and any_infec2 = 1 then parasitic_infec2 = 1; 
	else if parasitic_infec = 1 and any_infec2 = 0 then parasitic_infec2 = 0; 
	else if parasitic_infec = 0 then parasitic_infec2 = 0; 

	if other_infec = 1 and any_infec2 = 1 then other_infec2 = 1; 
	else if other_infec = 1 and any_infec2 = 0 then other_infec2 = 0; 
	else if other_infec = 0 then other_infec2 = 0; 
run;

data steph.timevarcohort_27Jul23; set tot_pop_long3; run;

*******************************************************************;
*create mental and physical health covariates; 
*load population data;
*select first row of records (date order = 1) - this will be the only row for those without an infection. 
For the first row, the stop date is either the infection date, the dementia date, or the censor date;
data totpop; set steph.timevarcohort_27Jul23; where date_order = 1; run;

*load mental and physical health dx data - data set up in code file PhysMentalHealth_DiagnosticData;
data mh_dx; set steph.MHdx_27Jul22; run;
data ph_dx; set steph.PHDX_27JUL22; run;

*check dates;
proc sort data=mh_dx; 
by hosp_start;
run;

proc sort data=ph_dx; 
by hosp_start;
run;

proc sort data=totpop;
by start_date; 
run;

*select first_event of MH dx;
proc sort data=mh_dx; 
by snz_uid hosp_start;
run;
data mh_dx1; set mh_dx;
by snz_uid;
if first.snz_uid;
run;

*left join MH with pop; 
proc sql; 
	create table totpop_mh as 
	select a.*, b.hosp_start as mh_start 
		from totpop a 
		left join mh_dx1 b
			on a.snz_uid=b.snz_uid;
quit;

*check that there are no duplicate - all good;
proc sql;
...
*select first_event of PH dx;
proc sort data=ph_dx; 
by snz_uid hosp_start;
run;
data ph_dx1; set ph_dx;
by snz_uid;
if first.snz_uid;
run;

*left join PH with pop; 
proc sql; 
	create table totpop_alldx as 
	select a.*, b.hosp_start as ph_start 
		from totpop_mh a 
		left join ph_dx1 b
			on a.snz_uid=b.snz_uid;
quit;

*create covariate flags; 
data totpop_cov; set totpop_alldx; 
if mh_start ne . and mh_start < stop_date then mh_flag = 1; else mh_flag = 0;
if ph_start ne . and ph_start < stop_date then ph_flag = 1; else ph_flag = 0;
run;

*merge back in with original long format data;
proc sql;
	create table totpop_long as 
	select a.*, b.mh_start, b.mh_flag, b.ph_start, b.ph_flag
		from steph.timevarcohort_27Jul23 a
		left join totpop_cov b
			on a.snz_uid = b.snz_uid; 
quit;

*save file; 
data steph.timevarcohort_4AUG23; set totpop_long; 
run; 
